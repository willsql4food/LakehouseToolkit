...
# DBTITLE 1,PMAX - Fiscal Month
# MAGIC %sql
# MAGIC /* Tanner's original query
# MAGIC --PMAX QUERY--
# MAGIC SELECT    e.device__category,
# MAGIC           CASE 
# MAGIC           WHEN contains(e.session_traffic_source_last_click__google_ads_campaign__campaign_name, '|PLA')
# MAGIC           THEN 'PLA'
# MAGIC           ELSE 'LIA'
# MAGIC           END AS PMAX_Channel,
...
# MAGIC   and   ( -- 9-digit product ID immediately before HMTL
# MAGIC           regexp_instr(ep.string_val, '[0-9][0-9][0-9][0-9][0-9][0-9][0-9][0-9][0-9]\.html') > 0    
# MAGIC           -- Various alphaNNN patterns
...
# MAGIC     ,     ep.string_val
# MAGIC     ,     e.session_traffic_source_last_click__google_ads_campaign__campaign_name
...
# MAGIC %sql
# MAGIC /* Turn this into a Silver table with a daily summary per url */
# MAGIC --create table if not exists silver_dev.googleanalytics.pmax_page_views as
# MAGIC insert into silver_dev.googleanalytics.pmax_page_views 
# MAGIC SELECT    e.event_date,
# MAGIC           e.device__category,
# MAGIC           CASE 
# MAGIC           WHEN contains(e.session_traffic_source_last_click__google_ads_campaign__campaign_name, '|PLA')
# MAGIC           THEN 'PLA'
# MAGIC           ELSE 'LIA'
...
# MAGIC FROM      bronze_dev.googleanalytics.ga_events e
# MAGIC join      bronze_dev.googleanalytics.ga_events_event_params ep on ep.join_key = e.join_key
# MAGIC WHERE     e.event_name = 'page_view'
# MAGIC   and     ep.key = 'page_location'
# MAGIC   and     e.geo__country in ('Mexico', 'United States', 'Canada')
# MAGIC   and   ( -- 9-digit product ID immediately before HMTL
# MAGIC           regexp_instr(ep.string_val, '[0-9][0-9][0-9][0-9][0-9][0-9][0-9][0-9][0-9]\.html') > 0    
...
# MAGIC       or  regexp_instr(ep.string_val, 'SPP[0-9][0-9][0-9]') > 0
# MAGIC       or  regexp_instr(ep.string_val, 'SV[0-9][0-9][0-9]') > 0 )                            
# MAGIC   -- PMAX Channel Filter
# MAGIC   and contains (e.session_traffic_source_last_click__google_ads_campaign__campaign_name, '|PMAX|') 
# MAGIC   -- Processing Date Filter
# MAGIC   and     e.event_date BETWEEN :start_date and :end_date --Fiscal Month
# MAGIC GROUP BY  e.event_date
# MAGIC     ,     e.device__category
# MAGIC     ,     ep.string_val
# MAGIC     ,     e.session_traffic_source_last_click__google_ads_campaign__campaign_name
# MAGIC order BY  e.device__category
...
# MAGIC %sql
# MAGIC select    PMAX_Channel, event_date, sum(num_page_views) 
# MAGIC from      silver_dev.googleanalytics.pmax_page_views
# MAGIC group by  PMAX_Channel, event_date
# MAGIC order by  event_date desc, PMAX_Channel
