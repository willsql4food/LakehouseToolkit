create procedure dbo.uspGetGbqLoadBatchesWithMapping (@RowsLimit int = 1e6, @PipelineExecutionId int = 0)
/*  ===================================================================================================================
    When            Who                     What
    --------------  ---------------------   ---------------------------------------------------------------------------
    ...
    2024-02-26      A. Carter Burleigh      FORK from uspGetGbqLoadBatches
											Added schema expansion to generate ADF translate JSON directive
...
begin
    ...
    */
    ...
    select      wm.TableCatalog
            ...
            ,   SelectCmd = concat('select ', string_agg(src.source, ', ') within group (order by ColumnId, [position])
    ...
    /* Build the JSON - an outer to set the type with the mappings array */
    cross apply(select      [type] = 'TabularTranslator', j2.mappings
                from    (   /* And the inner to gather the source and sink names of the expanded columns into path & name */
                            select      [source.path] = snk.Source, [sink.name] = snk.Sink
                            from        cols snk
                            where       snk.TableCatalog = src.TableCatalog 
                                and     snk.TableSchema = src.TableSchema 
                                and     snk.TableName = src.TableName
                            order by    snk.ColumnId, snk.[position]
